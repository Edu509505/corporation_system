...
function Grafico() {
    ...
    // const { data: itensDaQuantitativa } = useSuspenseQuery({
    //     queryKey: ["itensDaQuantitativa"],
    //     queryFn: async () => {
    //         const response = await fetch(`${url}/quantitativaUnica/${itensDoDia.map((item) => item.idQuantitativa)}`)
    //         if (!response.ok) throw new Error("erro ao encontrar Quantitativa");
    //         const data = await response.json();
    //         return data as todosOsItensDoDia[]
    //     }
    // })
